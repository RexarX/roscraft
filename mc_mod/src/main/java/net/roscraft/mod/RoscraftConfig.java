 ...
 *
 * <p>
 * Missing or corrupt config files are replaced with defaults and re-written to
 * disk so the user has a template to edit.
 *
 * <p>
 * All fields are intentionally package-private; access is via the record
...
public record RoscraftConfig(String bridgeType, String networkHost, int networkPort) {
  ...
  public RoscraftConfig {
    bridgeType =
        Objects.requireNonNull(bridgeType, "bridgeType must not be null").toLowerCase(Locale.ROOT);
    networkHost =
        Objects.requireNonNull(networkHost, "networkHost must not be null").trim();
    ...
    if (networkPort < 1 || networkPort > 65_535) {
      throw new IllegalArgumentException("networkPort must be in [1, 65535], got: " + networkPort);
    }
  }
