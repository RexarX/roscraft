...
/**
 * Creates the correct {@link RoscraftBridge} implementation based on the active
 * {@link RoscraftConfig}.
 */
public final class BridgeFactory {
   ...
   * @param config
   *            Mod configuration.
   * @return A fully constructed bridge ready to receive
   *         {@link net.roscraft.bridge.BridgeCallback} registrations.
   * @throws IllegalStateException
   *             if the native library cannot be loaded (JNI mode) or the UDP
  ...
  public static RoscraftBridge create(RoscraftConfig config) {
    ...
    if (config.isJni()) {
      RoscraftMod.LOGGER.warn("JNI bridge requested, but native library is unavailable; "
          + "falling back to network bridge.");
    }

    RoscraftMod.LOGGER.info(
        "Using network bridge → udp://{}:{}", config.networkHost(), config.networkPort());
    return new NetworkBridge(new NetworkBridge.Config(config.networkHost(), config.networkPort()));
  }
}
