...
public final class RoscraftConfigScreen extends Screen {
  ...
  protected void init() {
    ...
    portField = this.addDrawableChild(new TextFieldWidget(
        this.textRenderer,
        left,
        top + 96,
        fieldWidth,
    ...
    this.addDrawableChild(
        ButtonWidget.builder(Text.translatable("text.roscraft.config.save"), button -> save())
            .dimensions(this.width / 2 - 104, top + 144, 100, 20)
            .build());

    this.addDrawableChild(
        ButtonWidget.builder(Text.translatable("text.roscraft.config.cancel"), button -> close())
  ...
  public void render(DrawContext context, int mouseX, int mouseY, float delta) {
    ...
    context.drawTextWithShadow(
        this.textRenderer, Text.translatable("text.roscraft.config.mode"), left, top, 0xA0A0A0);
  ...
  private Text modeButtonText() {
    Text modeText = "network".equals(bridgeType)
