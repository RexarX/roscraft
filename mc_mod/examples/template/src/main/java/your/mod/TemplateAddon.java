 ...
 * }</pre>
 */
public class TemplateAddon extends AbstractRoscraftAddon implements RoscraftAddonCommands {
  ...
  protected void configure() {
    on(BridgeEvent.TopicPayload.class, p -> ctx.ifBridgeConnected(bridge -> bridge.topics().publish(
        "/roscraft/example/out",
        "std_msgs/msg/String",
        String.format("data: 'echo: topic=%s bytes=%d'", p.topicName(), p.payloadLength())
            .getBytes(StandardCharsets.UTF_8))));

    on(BridgeEvent.BridgeError.class, e -> ctx.ifBridgeConnected(bridge -> bridge.topics().publish(
        "/roscraft/example/out",
        "std_msgs/msg/String",
        String.format("data: 'error: code=%s msg=%s'", e.errorCode(), e.errorMessage())
            .getBytes(StandardCharsets.UTF_8))));
  }
        ...
        CommandManager.literal("template")
            ...
            .then(CommandManager.literal("pub")
                .then(CommandManager.argument("count", IntegerArgumentType.integer(1, 100))
                    .executes(c -> publishN(
                        c.getSource(), IntegerArgumentType.getInteger(c, "count"))))));
  }

  private int help(ServerCommandSource src) {
    src.sendMessage(Text.literal("/ros template pub <count> — publish N messages"));
    return 1;
  }

  private int publishN(ServerCommandSource src, int count) {
    if (!ctx.isBridgeConnected()) {
      src.sendMessage(Text.literal("[Roscraft] ")
    ...
    var bridge = ctx.bridgeIfConnected().orElseThrow();
    for (int i = 0; i < count; i++) {
      bridge.topics().publish(
          "/roscraft/example/out",
          "std_msgs/msg/String",
          String.format("data: 'template #%d'", i).getBytes(StandardCharsets.UTF_8));
    }
    src.sendMessage(Text.literal("[Roscraft] ")
        .formatted(Formatting.AQUA)
        .append(Text.literal("Published " + count + " messages.").formatted(Formatting.GREEN)));
    return 1;
  }
}
