...
class ParamGetNode final : public rclcpp::Node {
public:
  ...
  ParamGetNode(CommandQueue& incoming, CommandQueue& outgoing,
               std::nullptr_t) = delete;

  ParamGetNode(const ParamGetNode&) = delete;
  ParamGetNode(ParamGetNode&&) = delete;
  ~ParamGetNode() override = default;

  ParamGetNode& operator=(const ParamGetNode&) = delete;
  ParamGetNode& operator=(ParamGetNode&&) = delete;

private:
  /// @brief Drain pending `ParamGetCmd` commands.
  void DrainParamGetCommands();
  ...
  CommandQueueConsumerToken param_get_consumer_;
  CommandQueueProducerToken param_get_response_producer_;
