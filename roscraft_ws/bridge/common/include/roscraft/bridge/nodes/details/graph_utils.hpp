 ...
 */
[[nodiscard]] constexpr bool IsHiddenName(std::string_view name) {
  ...
  while (segment_begin <= name.size()) {
    ...
    const auto segment = name.substr(segment_begin, count);
