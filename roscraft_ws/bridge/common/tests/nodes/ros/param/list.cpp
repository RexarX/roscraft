...
void RegisterQueues(CommandQueue& incoming, CommandQueue& outgoing) {
  incoming.Register<ParamListCmd>();
  outgoing.Register<ParamListResponseCmd>();
  outgoing.Register<ErrorCmd>();
}

auto MakeParamTargetNode() -> std::shared_ptr<rclcpp::Node> {
  auto node = std::make_shared<rclcpp::Node>("param_target_list_node");
...
TEST_SUITE("bridge::ParamListNode") {
  TEST_CASE("bridge::ParamListNode::ctor") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);

    ParamListNode node(incoming, outgoing, std::pmr::get_default_resource());

    CHECK(std::string_view(node.get_name()) == "roscraft_param_list_node");
    CHECK_NE(node.poll_timer_, nullptr);
  }

  TEST_CASE("bridge::ParamListNode::DrainParamListCommands") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
    auto node = std::make_shared<ParamListNode>(
    ...
    SUBCASE("Valid input") {
      ...
      ParamListCmd cmd(std::pmr::get_default_resource());
      cmd.request_id = 12;
      cmd.node_name = "/param_target_list_node";
      cmd.filter_regex = "^integer_value$";
      cmd.include_types = true;
      ...
      CHECK_EQ(response.request_id, 12);
      CHECK_EQ(response.node_name, "/param_target_list_node");
      REQUIRE_EQ(response.names.size(), 1);
      CHECK_EQ(response.names.front(), "integer_value");
      REQUIRE_EQ(response.types.size(), 1);
      CHECK_FALSE(response.types.front().empty());
      CHECK_FALSE(outgoing.HasCommands<ErrorCmd>());
    }
  }

  TEST_CASE("bridge::ParamListNode::OnPollTimer") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
  ...
  TEST_CASE("bridge::ParamListNode::SendError") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
