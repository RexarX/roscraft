...
void RegisterQueues(CommandQueue& incoming, CommandQueue& outgoing) {
  incoming.Register<ParamGetCmd>();
  outgoing.Register<ParamGetResponseCmd>();
  outgoing.Register<ErrorCmd>();
}

auto MakeParamTargetNode() -> std::shared_ptr<rclcpp::Node> {
  auto node = std::make_shared<rclcpp::Node>("param_target_get_node");
...
TEST_SUITE("bridge::ParamGetNode") {
  TEST_CASE("bridge::ParamGetNode::ctor") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);

    ParamGetNode node(incoming, outgoing, std::pmr::get_default_resource());

    CHECK(std::string_view(node.get_name()) == "roscraft_param_get_node");
    CHECK_NE(node.poll_timer_, nullptr);
  }

  TEST_CASE("bridge::ParamGetNode::DrainParamGetCommands") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
    auto node = std::make_shared<ParamGetNode>(
        incoming, outgoing, std::pmr::get_default_resource());

    SUBCASE("Invalid input") {
      ParamGetCmd cmd(std::pmr::get_default_resource());
      cmd.request_id = 21;
      cmd.node_name = "/param_target_get_node";
      cmd.param_name = "";
      incoming.Enqueue(std::move(cmd));

      node->DrainParamGetCommands();

      ErrorCmd err(std::pmr::get_default_resource());
      CHECK(outgoing.Dequeue(err));
      CHECK_EQ(err.request_id, 21);
      CHECK_EQ(err.error_code, "PARAM_GET_FAILED");
    ...
    SUBCASE("Valid input") {
      ...
      CHECK_EQ(response.request_id, 22);
      CHECK_EQ(response.node_name, "/param_target_get_node");
      CHECK_EQ(response.param_name, "string_value");
      CHECK(response.found);
      CHECK_NE(response.value_text.find("hello"), std::string::npos);
  ...
  TEST_CASE("bridge::ParamGetNode::OnPollTimer") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
  ...
  TEST_CASE("bridge::ParamGetNode::SendError") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
