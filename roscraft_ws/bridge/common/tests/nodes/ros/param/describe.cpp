...
void RegisterQueues(CommandQueue& incoming, CommandQueue& outgoing) {
  incoming.Register<ParamDescribeCmd>();
  outgoing.Register<ParamDescribeResponseCmd>();
  outgoing.Register<ErrorCmd>();
}

auto MakeParamTargetNode() -> std::shared_ptr<rclcpp::Node> {
  auto node = std::make_shared<rclcpp::Node>("param_target_describe_node");
...
TEST_SUITE("bridge::ParamDescribeNode") {
  TEST_CASE("bridge::ParamDescribeNode::ctor") {
    ScopedRosContext context;
    ...
    CHECK(std::string_view(node.get_name()) == "roscraft_param_describe_node");
    CHECK_NE(node.poll_timer_, nullptr);
  }

  TEST_CASE("bridge::ParamDescribeNode::DrainParamDescribeCommands") {
    ScopedRosContext context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
    ...
    SUBCASE("Invalid input") {
      ...
      cmd.request_id = 41;
      cmd.node_name = "/param_target_describe_node";
      cmd.param_name = "";
      ...
      CHECK_EQ(err.error_code, "PARAM_DESCRIBE_FAILED");
    ...
    SUBCASE("Valid input") {
      ...
      cmd.request_id = 42;
      cmd.node_name = "/param_target_describe_node";
      cmd.param_name = "described_value";
      ...
      CHECK_EQ(response.node_name, "/param_target_describe_node");
      CHECK_EQ(response.param_name, "described_value");
      CHECK(response.found);
      CHECK_FALSE(response.param_type.empty());
      CHECK_EQ(response.description, "described parameter");
      CHECK_FALSE(response.read_only);
      CHECK_FALSE(outgoing.HasCommands<ErrorCmd>());
    }
  }

  TEST_CASE("bridge::ParamDescribeNode::OnPollTimer") {
    ScopedRosContext context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
  ...
  TEST_CASE("bridge::ParamDescribeNode::SendError") {
    ScopedRosContext context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
