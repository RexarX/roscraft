...
void RegisterQueues(CommandQueue& incoming, CommandQueue& outgoing) {
  incoming.Register<ParamDumpCmd>();
  outgoing.Register<ParamDumpResponseCmd>();
  outgoing.Register<ErrorCmd>();
}

auto MakeParamTargetNode() -> std::shared_ptr<rclcpp::Node> {
  auto node = std::make_shared<rclcpp::Node>("param_target_dump_node");
...
TEST_SUITE("bridge::ParamDumpNode") {
  TEST_CASE("bridge::ParamDumpNode::ctor") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);

    ParamDumpNode node(incoming, outgoing, std::pmr::get_default_resource());

    CHECK(std::string_view(node.get_name()) == "roscraft_param_dump_node");
    CHECK_NE(node.poll_timer_, nullptr);
  }

  TEST_CASE("bridge::ParamDumpNode::DrainParamDumpCommands") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
    auto node = std::make_shared<ParamDumpNode>(
    ...
    SUBCASE("Invalid input") {
      ...
      node->DrainParamDumpCommands();

      ErrorCmd err(std::pmr::get_default_resource());
      CHECK(outgoing.Dequeue(err));
      CHECK_EQ(err.request_id, 51);
      CHECK_EQ(err.error_code, "PARAM_DUMP_FAILED");
    ...
    SUBCASE("Valid input") {
      ...
      ParamDumpCmd cmd(std::pmr::get_default_resource());
      cmd.request_id = 52;
      cmd.node_name = "/param_target_dump_node";
      cmd.timeout_seconds = 2.0;
      incoming.Enqueue(std::move(cmd));

      node->DrainParamDumpCommands();

      ParamDumpResponseCmd response(std::pmr::get_default_resource());
      CHECK(outgoing.Dequeue(response));
      CHECK_EQ(response.request_id, 52);
      CHECK_EQ(response.node_name, "/param_target_dump_node");
      CHECK_NE(response.yaml_text.find("integer_value:"), std::string::npos);
      CHECK_NE(response.yaml_text.find("string_value:"), std::string::npos);
      CHECK_FALSE(outgoing.HasCommands<ErrorCmd>());
    }
  }

  TEST_CASE("bridge::ParamDumpNode::OnPollTimer") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
  ...
  TEST_CASE("bridge::ParamDumpNode::SendError") {
    ScopedRosContext ros_context;
    CommandQueue incoming;
    CommandQueue outgoing;
    RegisterQueues(incoming, outgoing);
