...
void NodeInfoNode::DrainNodeInfoCommands() {
  ...
  NodeInfoCmd cmd(allocator_);
  while (in_storage.Dequeue(node_info_consumer_, cmd)) {
    NodeInfoResponseCmd response(allocator_);
    ...
    const auto unique_publishers =
        std::ranges::unique(response.publishers, same_name_and_type);
    response.publishers.erase(unique_publishers.begin(),
                              response.publishers.end());

    std::ranges::sort(response.subscribers, by_name_and_type);
    const auto unique_subscribers =
        std::ranges::unique(response.subscribers, same_name_and_type);
    response.subscribers.erase(unique_subscribers.begin(),
                               response.subscribers.end());

    std::ranges::sort(response.services, by_name_and_type);
    const auto unique_services =
        std::ranges::unique(response.services, same_name_and_type);
    response.services.erase(unique_services.begin(), response.services.end());
