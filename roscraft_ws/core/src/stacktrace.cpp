...
#include <roscraft/utils/filesystem.hpp>
...
template <typename Trace>
[[nodiscard]] size_t FindStartingFrame(
    const Trace& stack_trace, const roscraft::StacktraceConfig& config) {
  ...
  try {
    if (has_source_file) {
      ...
      source_file_name =
          std::string(roscraft::utils::GetFileName(config.source_file));
    }
...
[[nodiscard]] size_t ComputeEndFrame(const Trace& stack_trace,
                                     const roscraft::StacktraceConfig& config,
                                     size_t start_frame) {
