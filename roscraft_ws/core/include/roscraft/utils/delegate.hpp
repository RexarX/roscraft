...
public:
  ...
  template <typename... UArgs>
    requires(sizeof...(UArgs) == sizeof...(Args)) &&
            (... && PolymorphicConvertible<UArgs, Args>)
...
template <auto Func>
  requires(
      ...
      (sizeof...(Args) ==
       std::tuple_size_v<
           typename details::FreeFunctionTraits<decltype(Func)>::Arguments>))
constexpr auto Delegate<ReturnType(Args...)>::FromFunction() noexcept
    -> Delegate {
  ...
  static_assert(
      []<std::size_t... I>(std::index_sequence<I...>) {
        return (
            ... &&
            PolymorphicConvertible<Args, std::tuple_element_t<I, Arguments>>);
...
constexpr auto DelegateFromFunction() noexcept {
  ...
  return []<std::size_t... I>(std::index_sequence<I...>) {
    return Delegate<ReturnType(
        std::tuple_element_t<I, Args>...)>::template FromFunction<Func>();
  }(std::make_index_sequence<std::tuple_size_v<Args>>{});
}

/**
 * @brief Helper to create delegate from free function pointer with explicit
 * signature.
 * @tparam Signature Free function pointer type.
 * @tparam Func Free function pointer value.
 * @return Delegate bound to the given free function.
...
constexpr auto DelegateFromFunction() noexcept {
  ...
  return []<std::size_t... I>(std::index_sequence<I...>) {
    return Delegate<ReturnType(std::tuple_element_t<I, Args>...)>::
        template FromFunction<Signature, Func>();
  }(std::make_index_sequence<std::tuple_size_v<Args>>{});
}
...
template <auto Func>
  requires std::is_member_function_pointer_v<decltype(Func)>
constexpr auto DelegateFromFunction(
    typename details::MemberFunctionTraits<decltype(Func)>::Class&
        instance) noexcept {
  using Traits = details::MemberFunctionTraits<decltype(Func)>;
  using ReturnType = typename Traits::ReturnType;
  ...
  return []<std::size_t... I>(std::index_sequence<I...>, auto& inst) {
    return Delegate<ReturnType(
        std::tuple_element_t<I, Args>...)>::template FromFunction<Func>(inst);
  }(std::make_index_sequence<std::tuple_size_v<Args>>{}, instance);
}

/**
 * @brief Helper to create delegate from member function pointer with explicit
 * signature.
 * @tparam Signature Member function pointer type.
 * @tparam Func Member function pointer value.
 * @param instance Reference to the object instance used for invocation.
...
constexpr auto DelegateFromFunction(
  ...
  return []<std::size_t... I>(std::index_sequence<I...>, auto& inst) {
    return Delegate<ReturnType(std::tuple_element_t<I, Args>...)>::
        template FromFunction<Signature, Func>(inst);
  }(std::make_index_sequence<std::tuple_size_v<Args>>{}, instance);
}
