 ...
 * index. Type identification uses `roscraft::utils::TypeIndex`.
 *
...
public:
   ...
   */
  [[nodiscard]] constexpr auto Data() noexcept -> MapType& { return storage_; }

  /**
   * @brief Returns a view of the underlying map (const).
   * @return Const reference to the underlying flat_map
   */
  [[nodiscard]] constexpr auto Data() const noexcept -> const MapType& {
    return storage_;
  }
...
constexpr Storage& MultiTypeMap<Storage, Allocator>::Ensure() {
  ...
  if (it == storage_.end()) {
    ...
    ROSCRAFT_ASSERT(success, "Failed to create storage for type '{}'!",
                    utils::TypeNameOf<T>());
