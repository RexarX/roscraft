...
#include <roscraft/container/details/typed_buffer_common.hpp>
...
inline T& TypedBufferArray<Allocator>::Emplace(size_type pos, Args&&... args) {
  ROSCRAFT_ASSERT(pos <= size_, "Emplace position '{}' is out of bounds!", pos);
  EnsureType<T>();
...
inline T* TypedBufferArray<Allocator>::Insert(size_type pos, size_type count,
  ...
  ROSCRAFT_ASSERT(pos <= size_, "Insert position '{}' is out of bounds!", pos);
...
inline auto TypedBufferArray<Allocator>::InsertRange(size_type pos,
  ...
  ROSCRAFT_ASSERT(pos <= size_, "InsertRange position '{}' is out of bounds!",
                  pos);
  EnsureType<T>();
...
constexpr void TypedBufferArray<Allocator>::PopBack() noexcept {
  ROSCRAFT_ASSERT(!Empty(), "Cannot pop from empty TypedBufferArray!");
  --size_;
...
constexpr void TypedBufferArray<Allocator>::Erase(size_type pos) {
  ROSCRAFT_ASSERT(pos < size_, "Erase position '{}' out of bounds!", pos);
  ROSCRAFT_ASSERT(HasType(), "Cannot erase from storage with no type!");
...
constexpr void TypedBufferArray<Allocator>::Erase(size_type first_pos,
                                                  size_type last_pos) {
  ROSCRAFT_ASSERT(first_pos <= size_,
                  "Erase range start '{}' is out of bounds!", first_pos);
  ROSCRAFT_ASSERT(last_pos <= size_, "Erase range end '{}' is out of bounds!",
                  last_pos);
  ROSCRAFT_ASSERT(first_pos <= last_pos, "Invalid erase range!");
...
constexpr void TypedBufferArray<Allocator>::Reserve(size_type count) {
  ...
  ROSCRAFT_ASSERT(HasType(), "Cannot reserve without a type set!");
...
inline T& TypedBufferArray<Allocator>::At(size_type index) noexcept {
  ROSCRAFT_ASSERT(index < size_, "Index '{}' is out of bounds!", index);
  ROSCRAFT_ASSERT(
...
inline T& TypedBufferArray<Allocator>::Front() noexcept {
  ROSCRAFT_ASSERT(!Empty(), "Cannot access front of empty TypedBufferArray!");
  ROSCRAFT_ASSERT(
      !type_info_.IsValid() || type_info_.type_index == TypeIndexOf<T>(),
      "Type mismatch: storage contains different type!");
  return *DataPtr<T>();
...
inline T& TypedBufferArray<Allocator>::Back() noexcept {
  ROSCRAFT_ASSERT(!Empty(), "Cannot access back of empty TypedBufferArray!");
  ROSCRAFT_ASSERT(
      !type_info_.IsValid() || type_info_.type_index == TypeIndexOf<T>(),
      "Type mismatch: storage contains different type!");
...
inline void TypedBufferArray<Allocator>::Swap(size_type index,
  ...
  ROSCRAFT_ASSERT(HasType(),
                  "Cannot swap elements in storage without a type set!");
  ROSCRAFT_ASSERT(index < size_, "Swap index '{}' is out of bounds!", index);
  ROSCRAFT_ASSERT(other_index < size_,
                  "Swap other_index '{}' is out of bounds!", other_index);
  ...
  if (type_info_.is_trivially_copyable) {
    ...
    ROSCRAFT_ASSERT(type_info_.move_construct,
...
inline void TypedBufferArray<Allocator>::Swap(size_type index,
  ...
  ROSCRAFT_ASSERT(HasType(),
                  "Cannot swap elements in storage without a type set!");
  ROSCRAFT_ASSERT(type_info_.type_index == TypeIndexOf<T>(),
...
constexpr void TypedBufferArray<Allocator>::GrowIfNeeded(
  ...
  ROSCRAFT_ASSERT(HasType(), "Cannot grow storage without a type set!");
...
constexpr void TypedBufferArray<Allocator>::ShiftRight(size_type pos,
                                                       size_type count) {
  ROSCRAFT_ASSERT(HasType(), "Cannot shift in storage without a type set!");
  ROSCRAFT_ASSERT(pos <= size_, "ShiftRight position '{}' is out of bounds!",
                  pos);
...
constexpr void TypedBufferArray<Allocator>::ShiftLeft(size_type pos,
                                                      size_type count) {
  ROSCRAFT_ASSERT(HasType(), "Cannot shift in storage without a type set!");
  ROSCRAFT_ASSERT(pos >= count, "ShiftLeft position '{}' is invalid!", pos);
  ROSCRAFT_ASSERT(pos <= size_, "ShiftLeft position '{}' is out of bounds!",
                  pos);
 ...
 */
template <roscraft::container::TypedBufferStorable T, typename Allocator>
  requires(
      std::is_class_v<Allocator> &&
      !std::same_as<
          std::remove_cvref_t<T>,
          typename roscraft::container::TypedBufferArray<Allocator>::size_type>)
inline auto erase(roscraft::container::TypedBufferArray<Allocator>& storage,
                  const T& value) ->
    typename roscraft::container::TypedBufferArray<Allocator>::size_type {
  using DecayedT = std::remove_cvref_t<T>;
  using size_type =
...
inline void erase(
    roscraft::container::TypedBufferArray<Allocator>& storage,
    typename roscraft::container::TypedBufferArray<Allocator>::size_type pos) {
...
inline auto erase_if(roscraft::container::TypedBufferArray<Allocator>& storage,
                     Pred pred) ->
    typename roscraft::container::TypedBufferArray<Allocator>::size_type {
  using size_type =
      typename roscraft::container::TypedBufferArray<Allocator>::size_type;
