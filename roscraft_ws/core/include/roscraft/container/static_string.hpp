...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    std::basic_string_view<CharT, Traits> sv) noexcept {
  ROSCRAFT_ASSERT(sv.size() <= StrCapacity,
                  "String view size exceeds capacity!");
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ROSCRAFT_ASSERT(pos <= sv.size(), "Position out of range!");
  const size_type substr_len = std::min(count, sv.size() - pos);
  ROSCRAFT_ASSERT(substr_len <= StrCapacity,
                  "Substring size exceeds capacity!");
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ROSCRAFT_ASSERT(copy_len <= StrCapacity,
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ROSCRAFT_ASSERT(count <= StrCapacity, "Count exceeds capacity!");
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ROSCRAFT_ASSERT(len <= StrCapacity, "Length exceeds capacity!");
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ROSCRAFT_ASSERT(ilist.size() <= StrCapacity,
...
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Insert(
    size_type pos, std::basic_string_view<CharT, Traits> sv) noexcept
    -> BasicStaticString& {
  ROSCRAFT_ASSERT(pos <= size_, "Position out of range!");
  ROSCRAFT_ASSERT(size_ + sv.size() <= StrCapacity,
...
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Insert(
  ...
  ROSCRAFT_ASSERT(pos <= size_, "Position out of range!");
  ROSCRAFT_ASSERT(size_ + count <= StrCapacity,
                  "Insert would exceed capacity!");
...
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::InsertRange(
  ...
  if constexpr (std::ranges::sized_range<Range>) {
    ...
    ROSCRAFT_ASSERT(size_ + range_size <= StrCapacity,
                    "Insert would exceed capacity!");
    ...
    auto insert_pos = pos;
    for (auto&& ch : std::forward<Range>(range)) {
      ROSCRAFT_ASSERT(size_ < StrCapacity, "Insert would exceed capacity!");
      if (insert_pos < size_) {
...
constexpr void BasicStaticString<StrCapacity, CharT, Traits>::PushBack(
    CharT ch) noexcept {
  ROSCRAFT_ASSERT(size_ < StrCapacity,
                  "Cannot PushBack: string is at capacity!");
  data_[size_++] = ch;
  data_[size_] = CharT{};
}
...
BasicStaticString<StrCapacity, CharT, Traits>::PopBack() noexcept {
  ROSCRAFT_ASSERT(!Empty(), "Cannot PopBack: string is empty!");
  data_[--size_] = CharT{};
}
...
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Replace(
  ...
  ROSCRAFT_ASSERT(new_size <= StrCapacity, "Replace would exceed capacity!");
...
constexpr void BasicStaticString<StrCapacity, CharT, Traits>::Resize(
    size_type count, CharT ch) noexcept {
  ROSCRAFT_ASSERT(count <= StrCapacity, "Resize count exceeds capacity!");
  if (count > size_) {
...
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Front() noexcept
    -> reference {
  ROSCRAFT_ASSERT(!Empty(), "String is empty!");
  return data_[0];
}
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ROSCRAFT_ASSERT(pos <= other.Size(), "Position out of range!");
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    const BasicStaticString<OtherCapacity, CharT, Traits>& other) noexcept {
  ROSCRAFT_ASSERT(other.Size() <= StrCapacity,
...
struct hash<
  ...
  [[nodiscard]] constexpr size_t operator()(
      const roscraft::container::BasicStaticString<StrCapacity, CharT, Traits>&
          str) const noexcept {
