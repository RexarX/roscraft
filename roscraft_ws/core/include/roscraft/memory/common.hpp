...
[[nodiscard]] constexpr size_t AlignUp(size_t value,
  ...
  ROSCRAFT_ASSERT(IsPowerOfTwo(alignment),
                  "alignment '{}' must be power of two", alignment);
...
/**
 * @brief Fails fast for unrecoverable PMR allocation errors.
 * @warning Always terminates.
 * @param message Diagnostic message
 */
[[noreturn]] inline void PmrFailFast(std::string_view message) noexcept {
  ROSCRAFT_VERIFY(false, "PMR allocation failed: {}", message);
  std::abort();
}
