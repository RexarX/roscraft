 ...
 * @example
 * @code
 * class Mesh final : public roscraft::memory::RcFromThis<Mesh> {
 * public:
 ...
 * };
 *
 * auto mesh = roscraft::memory::MakeRc<Mesh>(42);
 * @endcode
 */
template <typename Derived>
class RcFromThis {
 ...
 * @example
 * @code
 * class Texture final : public roscraft::memory::ArcFromThis<Texture> {
 * public:
 ...
 * };
 *
 * auto tex = roscraft::memory::MakeArc<Texture>("diffuse");
 * @endcode
 */
template <typename Derived>
class ArcFromThis {
...
inline bool ArcFromThis<Derived>::Release() noexcept {
  ROSCRAFT_ASSERT(ref_count_.load(std::memory_order_relaxed) > 0,
                  "Release() called on object with zero ref count!");
 ...
 * auto mesh = roscraft::memory::MakeRc<Mesh, PoolAllocator<Mesh>>(42);
...
public:
  ...
  RefCounted(const RefCounted<Other, Allocator>&
  ...
  template <typename Other>
    requires std::convertible_to<Other*, Derived*>
  RefCounted(RefCounted<Other, Allocator>&&
                 other) noexcept;  // NOLINT(google-explicit-constructor)
...
template <typename Other>
  requires std::convertible_to<Other*, Derived*>
inline RefCounted<Derived, Allocator>::RefCounted(
    const RefCounted<Other, Allocator>&
        other) noexcept  // NOLINT(google-explicit-constructor)
    : ptr_(other.Get()),
      alloc_(std::allocator_traits<AllocatorType>::
...
inline Derived& RefCounted<Derived, Allocator>::operator*() const noexcept {
  ROSCRAFT_ASSERT(ptr_ != nullptr, "Dereferencing null RefCounted!");
  return *ptr_;
}
...
public:
  ...
  template <typename Other>
    requires std::convertible_to<Other*, Derived*>
  AtomicRefCounted(const AtomicRefCounted<Other, Allocator>&
                       other) noexcept;  // NOLINT(google-explicit-constructor)
  ...
  template <typename Other>
    requires std::convertible_to<Other*, Derived*>
  AtomicRefCounted(AtomicRefCounted<Other, Allocator>&&
                       other) noexcept;  // NOLINT(google-explicit-constructor)
...
inline Derived& AtomicRefCounted<Derived, Allocator>::operator*()
    const noexcept {
  ROSCRAFT_ASSERT(ptr_ != nullptr, "Dereferencing null AtomicRefCounted!");
  return *ptr_;
}
