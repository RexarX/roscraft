...
private:
  ...
  void do_deallocate(void* p, size_t bytes, size_t alignment) override {
    CurrentArena().deallocate(p, bytes, alignment);
  }
...
inline ArenaAllocator& NFrameAllocator<N>::Arena(size_t index) noexcept {
  ROSCRAFT_ASSERT(index < N, "index '{}' is out of range [0, {})", index, N);
...
inline auto NFrameAllocator<N>::BuildArenas(
    const FrameAllocatorOptions& options, std::index_sequence<Indexes...>)
    -> std::array<ArenaAllocator, N> {
