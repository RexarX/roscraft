...
#include <roscraft/container/typed_buffer_array.hpp>
...
TEST_SUITE("container::TypedBufferStorable") {
...
TEST_SUITE("container::TypedBufferArray") {
  ...
  TEST_CASE("container::TypedBufferArray::std::erase and std::erase_if") {
    SUBCASE("std::erase removes matching elements") {
      TypedBufferArray buffer;
      buffer.PushBack(1);
      buffer.PushBack(2);
      buffer.PushBack(3);
      ...
      auto erased = std::erase(buffer, 2);
      size_t i = buffer.Size();
      CHECK_EQ(erased, 2);
      CHECK_EQ(i, 3);
    }

    SUBCASE("std::erase_if removes elements matching predicate") {
      TypedBufferArray buffer;
      buffer.PushBack(1);
      buffer.PushBack(2);
      buffer.PushBack(3);
      ...
      auto erased =
          std::erase_if<int>(buffer, [](const int& v) { return v % 2 == 0; });
      size_t i = buffer.Size();
      CHECK_EQ(erased, 2);
      CHECK_EQ(i, 3);
    }
