...
#include <roscraft/utils/delegate.hpp>

#include <concepts>

using namespace roscraft::utils;
...
TEST_SUITE("utils::Delegate") {
  TEST_CASE("utils::Delegate::ctor: Construction and basic state") {
    ...
    SUBCASE("Copy construction") {
      auto original =
          Delegate<int(int, int)>::FromFunction<&free_function_sum>();
      auto copy = original;
      CHECK(copy.Valid());
      CHECK_EQ(copy(2, 3), 5);
  ...
  TEST_CASE("utils::Delegate::FromFunction: Free function binding") {
    SUBCASE("Single parameter free function") {
      auto delegate = Delegate<int(int)>::FromFunction<&free_function_double>();
    ...
    SUBCASE("Void return type free function") {
      int result = 0;
      auto delegate = Delegate<void(int&)>::FromFunction<&free_function_void>();
      CHECK(delegate.Valid());
      delegate(result);
      CHECK_EQ(result, 42);
    }
    ...
    SUBCASE("Explicit signature template parameter for free function") {
      ...
      auto delegate =
          Delegate<int(int, int)>::FromFunction<Signature, func_ptr>();
      CHECK(delegate.Valid());
    ...
    SUBCASE("Constexpr delegate creation") {
      constexpr auto delegate =
          Delegate<int(int)>::FromFunction<&free_function_double>();
      CHECK(delegate.Valid());
      CHECK_EQ(delegate(6), 12);
    }
  }

  TEST_CASE("utils::Delegate::FromFunction: Member function binding") {
    SUBCASE("Non-const member function with no parameters") {
      Counter counter{10};
      auto delegate =
          Delegate<void()>::FromFunction<&Counter::increment>(counter);
      CHECK(delegate.Valid());
    ...
    SUBCASE("Non-const member function with parameters") {
      Counter counter{5};
      auto delegate = Delegate<void(int)>::FromFunction<&Counter::add>(counter);
      CHECK(delegate.Valid());
      delegate(10);
    ...
    SUBCASE("Non-const member function with return value") {
      Counter counter{7};
      auto delegate =
          Delegate<int(int)>::FromFunction<&Counter::multiply>(counter);
      CHECK(delegate.Valid());
    ...
    SUBCASE("Const member function") {
      ...
      auto delegate = Delegate<int()>::FromFunction<&Counter::get_value>(
    ...
    SUBCASE("Const member function with parameters") {
      ...
      auto delegate =
          Delegate<int(int)>::FromFunction<&ConstMemberStruct::multiply_const>(
    ...
    SUBCASE("Explicit signature template parameter for member function") {
      ...
      auto delegate =
          Delegate<void(int)>::FromFunction<Signature, func_ptr>(counter);
      CHECK(delegate.Valid());
      delegate(7);
  ...
  TEST_CASE("utils::Delegate::FromFunction: Overloaded member functions") {
    OverloadedStruct obj{10};

    SUBCASE("Zero-argument overload") {
      auto delegate = Delegate<int()>::FromFunction<
          static_cast<int (OverloadedStruct::*)()>(&OverloadedStruct::foo)>(
          obj);
      CHECK_EQ(delegate(), 10);
    }

    SUBCASE("Single-argument overload") {
      auto delegate = Delegate<int(int)>::FromFunction<
          static_cast<int (OverloadedStruct::*)(int)>(&OverloadedStruct::foo)>(
          obj);
      CHECK_EQ(delegate(5), 15);
    }

    SUBCASE("Two-argument overload") {
      auto delegate = Delegate<int(int, int)>::FromFunction<static_cast<int (
          OverloadedStruct::*)(int, int)>(&OverloadedStruct::foo)>(obj);
      CHECK_EQ(delegate(3, 7), 20);
    }
  }

  TEST_CASE("utils::Delegate::FromFunction: Virtual function dispatch") {
    SUBCASE("Virtual function called through base reference") {
      DerivedVirtual derived;
      BaseVirtual& base = derived;
      auto delegate =
          Delegate<int(int)>::FromFunction<&BaseVirtual::compute>(base);
    ...
    SUBCASE("Const virtual function") {
      ...
      auto delegate =
          Delegate<int(int)>::FromFunction<&BaseVirtual::compute_const>(
  ...
  TEST_CASE("utils::Delegate::Invoke: Empty delegate behavior") {
  ...
  TEST_CASE("utils::Delegate::operator==: Comparison operators") {
    ...
    SUBCASE("Different free function delegates are not equal") {
      auto delegate1 =
          Delegate<int(int)>::FromFunction<&free_function_double>();
      auto delegate2 =
          Delegate<int(int)>::FromFunction<&free_function_triple>();
    ...
    SUBCASE("Same member function on same instance are equal") {
      Counter counter{5};
      auto delegate1 =
          Delegate<void(int)>::FromFunction<&Counter::add>(counter);
      auto delegate2 =
          Delegate<void(int)>::FromFunction<&Counter::add>(counter);
      CHECK_EQ(delegate1, delegate2);
    }
    ...
    SUBCASE("Same member function on different instances are not equal") {
      ...
      Counter counter2{5};
      auto delegate1 =
          Delegate<void(int)>::FromFunction<&Counter::add>(counter1);
      auto delegate2 =
          Delegate<void(int)>::FromFunction<&Counter::add>(counter2);
      CHECK_NE(delegate1, delegate2);
    }
  ...
  TEST_CASE("utils::Delegate::Reset") {
  ...
  TEST_CASE("utils::DelegateFromFunction: Helper functions") {
    SUBCASE("DelegateFromFunction for free function") {
      auto delegate = DelegateFromFunction<&free_function_sum>();
    ...
    SUBCASE("DelegateFromFunction for single parameter free function") {
      auto delegate = DelegateFromFunction<&free_function_double>();
    ...
    SUBCASE("DelegateFromFunction for member function") {
      Counter counter{4};
      auto delegate = DelegateFromFunction<&Counter::multiply>(counter);
      CHECK(std::same_as<decltype(delegate), Delegate<int(int)>>);
      CHECK(delegate.Valid());
    ...
    SUBCASE("DelegateFromFunction constexpr") {
      constexpr auto delegate = DelegateFromFunction<&free_function_triple>();
      CHECK(delegate.Valid());
    ...
    SUBCASE("DelegateFromFunction with explicit signature for free function") {
      using Signature = int (*)(int, int);
      constexpr Signature func_ptr = &free_function_sum;
      auto delegate = DelegateFromFunction<Signature, func_ptr>();
      CHECK(std::same_as<decltype(delegate), Delegate<int(int, int)>>);
      CHECK(delegate.Valid());
    ...
    SUBCASE(
        "DelegateFromFunction with explicit signature for single param free "
        "function") {
    ...
    SUBCASE(
        "DelegateFromFunction with explicit signature for member function") {
      Counter counter{3};
      using Signature = int (Counter::*)(int);
      constexpr Signature func_ptr = &Counter::multiply;
      auto delegate = DelegateFromFunction<Signature, func_ptr>(counter);
      CHECK(std::same_as<decltype(delegate), Delegate<int(int)>>);
      CHECK(delegate.Valid());
    ...
    SUBCASE(
        "DelegateFromFunction with explicit signature for overloaded member "
        "function") {
      ...
      auto delegate = DelegateFromFunction<Signature, func_ptr>(obj);
      CHECK(std::same_as<decltype(delegate), Delegate<int(int)>>);
      CHECK(delegate.Valid());
    ...
    SUBCASE("DelegateFromFunction with explicit signature constexpr") {
      ...
      constexpr auto delegate = DelegateFromFunction<Signature, func_ptr>();
      CHECK(delegate.Valid());
      CHECK_EQ(delegate(5), 15);
    }
  }

  TEST_CASE("utils::Delegate::Invoke: Polymorphic conversion") {
    SUBCASE("Derived to base conversion in arguments") {
      ...
      // The implementation supports it, but direct testing requires internal
      // access
      CHECK_EQ(derived.get_id(), 2);
    }
  }

  TEST_CASE("utils::Delegate: Edge cases and special scenarios") {
