...
#include <roscraft/utils/defer.hpp>
...
TEST_SUITE("utils::Defer") {
  ...
  TEST_CASE("utils::DEFER_CALL::Multiple defers execute in LIFO order") {
    float x = 5.0F;
    {
      ...
      ROSCRAFT_DEFER_CALL(defer1);
      ROSCRAFT_DEFER_CALL(defer2);
    }
  ...
  TEST_CASE("utils::DEFER_CALL::Multiple defers on same line") {
    ...
    int b = 0;
    {
      auto defer_a = [&a]() { a = 1; };
      auto defer_b = [&b]() { b = 2; };
      ROSCRAFT_DEFER_CALL(defer_a);
      ROSCRAFT_DEFER_CALL(defer_b);
      CHECK_EQ(a, 0);
      CHECK_EQ(b, 0);
    }
    CHECK_EQ(a, 1);
    ...
    SUBCASE("Multiple defers all execute with exception") {
      ...
      CHECK_THROWS_AS(
          [&cleanup_count]() {
            ...
            ROSCRAFT_DEFER_CALL(d1);
            ROSCRAFT_DEFER_CALL(d2);
            ROSCRAFT_DEFER_CALL(d3);
            throw std::runtime_error("Test exception");
          }(),
          std::runtime_error);
  ...
  TEST_CASE("utils::DEFER_CALL::Functor object") {
    ...
    int x = 10;
    {
      ROSCRAFT_DEFER_CALL(Incrementer{x});
      CHECK_EQ(x, 10);
    }
    CHECK_EQ(x, 15);
  }
