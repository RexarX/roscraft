...
TEST_SUITE("utils::Random") {
  ...
  TEST_CASE("utils::RandomGenerator: different arithmetic types") {
    ...
    SUBCASE("char type") {
      auto value = generator.Value<char>();
      CHECK_GE(value, std::numeric_limits<char>::min());
      CHECK_LE(value, std::numeric_limits<char>::max());
    }

    SUBCASE("unsigned char type") {
      auto value = generator.Value<unsigned char>();
      CHECK_GE(value, std::numeric_limits<unsigned char>::min());
      CHECK_LE(value, std::numeric_limits<unsigned char>::max());
    }
