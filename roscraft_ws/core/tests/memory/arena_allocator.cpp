...
TEST_SUITE("memory::ArenaAllocator") {
  TEST_CASE("memory::ArenaAllocator::ctor(ArenaOptions)") {
  ...
  TEST_CASE("memory::ArenaAllocator::ctor(size_t)") {
  ...
  TEST_CASE("memory::ArenaAllocator::ctor(ArenaAllocator&&)") {
  ...
  TEST_CASE("memory::ArenaAllocator::operator=(ArenaAllocator&&)") {
  ...
  TEST_CASE("memory::ArenaAllocator::Reset") {
    SUBCASE("Empty returns true after Reset") {
      ArenaAllocator arena(1024);
      arena.allocate(128, kAlign);
    ...
    SUBCASE("BlockCount returns to 1 after Reset") {
      ...
      arena.allocate(kGrowCapacity, 1);
      arena.allocate(kGrowCapacity, 1);  // forces second block
    ...
    SUBCASE("TotalCapacity returns to InitialCapacity after Reset") {
      ...
      CHECK_EQ(arena.TotalCapacity(), arena.InitialCapacity());
    }

    SUBCASE("Stats counters are zeroed after Reset") {
      ArenaAllocator arena(1024);
      arena.allocate(64, kAlign);
  ...
  TEST_CASE("memory::ArenaAllocator::Empty") {
  ...
  TEST_CASE("memory::ArenaAllocator::Stats") {
  ...
  TEST_CASE("memory::ArenaAllocator::TotalCapacity") {
    ...
    SUBCASE("Increases when a second block is allocated due to overflow") {
      ...
      arena.allocate(kGrowCapacity, 1);
      arena.allocate(kGrowCapacity, 1);  // forces growth
  ...
  TEST_CASE("memory::ArenaAllocator::InitialCapacity") {
  ...
  TEST_CASE("memory::ArenaAllocator::BlockCount") {
  ...
  TEST_CASE("memory::ArenaAllocator::Growth") {
  ...
  TEST_CASE("memory::ArenaAllocator::allocate") {
  ...
  TEST_CASE("memory::ArenaAllocator::deallocate") {
  ...
  TEST_CASE("memory::ArenaAllocator thread-safety: concurrent allocate") {
  ...
  TEST_CASE("memory::ArenaAllocator thread-safety: concurrent deallocate") {
