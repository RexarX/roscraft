...
#include <roscraft/memory/ref_counted.hpp>
...
TEST_SUITE("memory::RefCounted") {
  TEST_CASE("memory::RefCounted::Default construction produces null handle") {
    Rc<Widget> rc;
    CHECK_FALSE(rc);
    CHECK(rc.Empty());
    CHECK_EQ(rc.Get(), nullptr);
    CHECK_EQ(rc.RefCount(), 0u);
  }

  TEST_CASE("memory::RefCounted::nullptr construction produces null handle") {
  ...
  TEST_CASE("memory::RefCounted::MakeRc constructs object with ref count 1") {
    auto rc = MakeRc<Widget>(42);

    CHECK(rc);
    CHECK_FALSE(rc.Empty());
    CHECK_NE(rc.Get(), nullptr);
    CHECK_EQ(rc.RefCount(), 1u);
    CHECK(rc.Unique());
    CHECK_EQ(rc->Value(), 42);
    CHECK_EQ((*rc).Value(), 42);
  }

  TEST_CASE("memory::RefCounted::Copy construction increments ref count") {
    auto rc1 = MakeRc<Widget>(10);
    CHECK_EQ(rc1.RefCount(), 1u);

    auto rc2 = rc1;
    CHECK_EQ(rc1.RefCount(), 2u);
    CHECK_EQ(rc2.RefCount(), 2u);
  ...
  TEST_CASE(
      "memory::RefCounted::Move construction transfers ownership without "
      "changing ref count") {
    ...
    CHECK_EQ(rc2.RefCount(), 1u);
    CHECK(rc2.Unique());
  }

  TEST_CASE(
      "memory::RefCounted::Destruction decrements ref count and deletes at "
      "zero") {
    int destroyed = 0;
    {
  ...
  TEST_CASE("memory::RefCounted::Last copy destroyed triggers deletion") {
    int destroyed = 0;
    Rc<Widget> rc2;
    {
  ...
  TEST_CASE("memory::RefCounted::Copy assignment increments ref count") {
  ...
  TEST_CASE("memory::RefCounted::Copy assignment to self is safe") {
  ...
  TEST_CASE("memory::RefCounted::Move assignment transfers ownership") {
  ...
  TEST_CASE("memory::RefCounted::Move assignment to self is safe") {
    ...
    (void)raw;
    CHECK_EQ(rc.RefCount(), rc.Empty() ? 0u : 1u);
  }

  TEST_CASE("memory::RefCounted::nullptr assignment resets handle") {
    int destroyed = 0;
  ...
  TEST_CASE("memory::RefCounted::Reset releases handle") {
    int destroyed = 0;
    auto rc = MakeRc<Widget>(0);
    rc->SetDestroyCounter(&destroyed);
    rc.Reset();
  ...
  TEST_CASE("memory::RefCounted::Reset on null handle is safe") {
    Rc<Widget> rc;
    rc.Reset();
    CHECK_FALSE(rc);
  }
  ...
  TEST_CASE(
      "memory::RefCounted::Release yields raw pointer and leaves handle null") {
    int destroyed = 0;
    ...
    CHECK_EQ(raw->RefCount(),
             1u);  // counter still at 1 — Release didn't decrement
    CHECK_EQ(destroyed, 0);
  ...
  TEST_CASE("memory::RefCounted::Equality operators") {
  ...
  TEST_CASE("memory::RefCounted::Multiple handles all share same ref count") {
    auto a = MakeRc<Node>(1);
    auto b = a;
    auto c = b;
    auto d = c;

    CHECK_EQ(a.RefCount(), 4u);
    CHECK_EQ(b.RefCount(), 4u);

    b.Reset();
    CHECK_EQ(a.RefCount(), 3u);
    c.Reset();
    CHECK_EQ(a.RefCount(), 2u);
    d.Reset();
    CHECK_EQ(a.RefCount(), 1u);
    CHECK(a.Unique());
  }

  TEST_CASE("memory::RefCounted::RcFromThis copy/move do not copy ref count") {
    auto rc = MakeRc<Widget>(0);
    CHECK_EQ(rc->RefCount(), 1u);

    Widget copy(*rc.Get());
    CHECK_EQ(copy.RefCount(), 0u);
  }
}

TEST_SUITE("memory::AtomicRefCounted") {
  TEST_CASE(
      "memory::AtomicRefCounted::Default construction produces null handle") {
    Arc<Texture> arc;
    CHECK_FALSE(arc);
    CHECK(arc.Empty());
    CHECK_EQ(arc.Get(), nullptr);
    CHECK_EQ(arc.RefCount(), 0u);
  }

  TEST_CASE(
      "memory::AtomicRefCounted::nullptr construction produces null handle") {
  ...
  TEST_CASE(
      "memory::AtomicRefCounted::MakeArc constructs object with ref count 1") {
    auto arc = MakeArc<Texture>(1);

    CHECK(arc);
    CHECK_FALSE(arc.Empty());
    CHECK_NE(arc.Get(), nullptr);
    CHECK_EQ(arc.RefCount(), 1u);
    CHECK(arc.Unique());
    CHECK_EQ(arc->Id(), 1);
    CHECK_EQ((*arc).Id(), 1);
  }
  ...
  TEST_CASE(
      "memory::AtomicRefCounted::Copy construction increments ref count "
      "atomically") {
    auto arc1 = MakeArc<Texture>(2);
    CHECK_EQ(arc1.RefCount(), 1u);

    auto arc2 = arc1;
    CHECK_EQ(arc1.RefCount(), 2u);
    CHECK_EQ(arc2.RefCount(), 2u);
    CHECK_EQ(arc1.Get(), arc2.Get());
    CHECK_FALSE(arc1.Unique());
  }

  TEST_CASE(
      "memory::AtomicRefCounted::Move construction transfers ownership without "
      "changing ref count") {
    ...
    CHECK_EQ(arc2.RefCount(), 1u);
    CHECK(arc2.Unique());
  }

  TEST_CASE(
      "memory::AtomicRefCounted::Destruction decrements ref count and deletes "
      "at zero") {
  ...
  TEST_CASE("memory::AtomicRefCounted::Last copy destroyed triggers deletion") {
    std::atomic<int> destroyed{0};
    Arc<Texture> arc2;
    {
  ...
  TEST_CASE("memory::AtomicRefCounted::Copy assignment increments ref count") {
  ...
  TEST_CASE("memory::AtomicRefCounted::Copy assignment to self is safe") {
  ...
  TEST_CASE("memory::AtomicRefCounted::Move assignment transfers ownership") {
  ...
  TEST_CASE("memory::AtomicRefCounted::nullptr assignment resets handle") {
    std::atomic<int> destroyed{0};
    auto arc = MakeArc<Texture>(0);
    arc->SetDestroyCounter(&destroyed);
  ...
  TEST_CASE("memory::AtomicRefCounted::Reset releases handle") {
    std::atomic<int> destroyed{0};
    auto arc = MakeArc<Texture>(0);
    arc->SetDestroyCounter(&destroyed);
    arc.Reset();
  ...
  TEST_CASE("memory::AtomicRefCounted::Reset on null handle is safe") {
    Arc<Texture> arc;
    arc.Reset();
    CHECK_FALSE(arc);
  }
  ...
  TEST_CASE(
      "memory::AtomicRefCounted::Release yields raw pointer and leaves handle "
      "null") {
    std::atomic<int> destroyed{0};
    auto arc = MakeArc<Texture>(0);
    arc->SetDestroyCounter(&destroyed);
  ...
  TEST_CASE("memory::AtomicRefCounted::Equality operators") {
  ...
  TEST_CASE(
      "memory::AtomicRefCounted::Multiple handles all share same ref count") {
  ...
  TEST_CASE(
      "memory::AtomicRefCounted::ArcFromThis copy/move do not copy ref count") {
    auto arc = MakeArc<Texture>(0);
    CHECK_EQ(arc->RefCount(), 1u);
  ...
  TEST_CASE(
      "memory::AtomicRefCounted::Concurrent copy-construction from shared "
      "handle is safe") {
    ...
    CHECK_EQ(shared.RefCount(), 1u);
    CHECK_EQ(copies_alive.load(), 0);
  }

  TEST_CASE(
      "memory::AtomicRefCounted::Concurrent destruction from multiple threads "
      "is safe") {
    constexpr int kThreadCount = 8;
    std::atomic<int> destroyed{0};
...
TEST_SUITE("memory::RcArcAliases") {
  TEST_CASE("memory::RcArcAliases::Rc<T> is RefCounted<T, std::allocator<T>>") {
    static_assert(
  ...
  TEST_CASE(
      "memory::RcArcAliases::Arc<T> is AtomicRefCounted<T, "
      "std::allocator<T>>") {
    static_assert(
...
TEST_SUITE("memory::MakeRcWithAllocator") {
  TEST_CASE(
      "memory::MakeRcWithAllocator::MakeRc without allocator argument requires "
  ...
  TEST_CASE(
      "memory::MakeRcWithAllocator::MakeRcWith — object is destroyed through "
  ...
  TEST_CASE(
      "memory::MakeRcWithAllocator::MakeRcWith — dealloc fires on last copy "
      "going out of scope") {
    AllocStats stats;
    int destroyed = 0;
  ...
  TEST_CASE(
      "memory::MakeRcWithAllocator::Copy of Rc propagates allocator (POCCA = "
      "true)") {
    AllocStats stats;
  ...
  TEST_CASE(
      "memory::MakeRcWithAllocator::Move of Rc propagates allocator (POCMA = "
      "true)") {
    AllocStats stats;
    auto rc1 = MakeRcWith<Widget>(TrackingAllocator<Widget>{stats}, 5);
  ...
  TEST_CASE(
      "memory::MakeRcWithAllocator::GetAllocator returns the allocator stored "
      "in the handle") {
    AllocStats stats;
...
TEST_SUITE("memory::MakeArcWithAllocator") {
  TEST_CASE(
      "memory::MakeArcWithAllocator::MakeArcWith — object is destroyed through "
      "the custom allocator") {
    AllocStats stats;
    std::atomic<int> destroyed{0};
    {
  ...
  TEST_CASE(
      "memory::MakeArcWithAllocator::MakeArcWith — dealloc fires on last copy "
      "going out of scope") {
    AllocStats stats;
    std::atomic<int> destroyed{0};
  ...
  TEST_CASE(
      "memory::MakeArcWithAllocator::Copy of Arc propagates allocator (POCCA = "
      "true)") {
    AllocStats stats;
  ...
  TEST_CASE(
      "memory::MakeArcWithAllocator::Move of Arc propagates allocator (POCMA = "
      "true)") {
    AllocStats stats;
    auto arc1 = MakeArcWith<Texture>(TrackingAllocator<Texture>{stats}, 1);
  ...
  TEST_CASE(
      "memory::MakeArcWithAllocator::GetAllocator returns the allocator stored "
      "in the handle") {
    AllocStats stats;
    auto arc = MakeArcWith<Texture>(TrackingAllocator<Texture>{stats}, 0);
  ...
  TEST_CASE(
      "memory::MakeArcWithAllocator::Concurrent destruction uses custom "
