...
TEST_SUITE("memory::NFrameAllocator") {
  TEST_CASE("memory::NFrameAllocator::ctor(FrameAllocatorOptions)") {
  ...
  TEST_CASE("memory::NFrameAllocator::ctor(size_t)") {
  ...
  TEST_CASE("memory::NFrameAllocator::ctor(NFrameAllocator&&)") {
  ...
  TEST_CASE("memory::NFrameAllocator::operator=(NFrameAllocator&&)") {
  ...
  TEST_CASE("memory::NFrameAllocator::Advance") {
  ...
  TEST_CASE("memory::NFrameAllocator::Reset") {
  ...
  TEST_CASE("memory::NFrameAllocator::Empty") {
  ...
  TEST_CASE("memory::NFrameAllocator::Stats") {
    ...
    SUBCASE("Stats show only current frame after Advance") {
      NFrameAllocator<2> alloc(BasicOptions());
      alloc.allocate(64, kAlign);  // frame 0
  ...
  TEST_CASE("memory::NFrameAllocator::AggregateStats") {
    ...
    SUBCASE("Sums total_allocations across all frame arenas") {
      ...
      alloc.allocate(32, kAlign);  // frame 0: 1 alloc
      alloc.allocate(32, kAlign);  // frame 0: 2 allocs
      alloc.Advance();
      alloc.allocate(64, kAlign);  // frame 1: 1 alloc
  ...
  TEST_CASE("memory::NFrameAllocator::FrameIndex") {
  ...
  TEST_CASE("memory::NFrameAllocator::FrameCount") {
  ...
  TEST_CASE("memory::NFrameAllocator::InitialCapacity") {
  ...
  TEST_CASE("memory::NFrameAllocator::Growth") {
  ...
  TEST_CASE("memory::NFrameAllocator::TotalCapacity") {
    ...
    SUBCASE("Grows when current arena grows") {
      ...
      alloc.allocate(64, 1);
      alloc.allocate(64, 1);  // forces growth in current arena

      CHECK_GT(alloc.TotalCapacity(), before);
    }
  }

  TEST_CASE("memory::NFrameAllocator::BlockCount") {
  ...
  TEST_CASE("memory::NFrameAllocator::Arena (mutable)") {
    SUBCASE("Returns mutable reference to the correct arena by index") {
      NFrameAllocator<3> alloc(BasicOptions());

      alloc.Arena(1).allocate(32, kAlign);
  ...
  TEST_CASE("memory::NFrameAllocator::Arena (const)") {
    SUBCASE("Const Arena returns correct Stats for each frame") {
      NFrameAllocator<2> alloc(BasicOptions());

      alloc.allocate(64, kAlign);  // frame 0: 1 alloc
      alloc.Advance();
      alloc.allocate(128, kAlign);  // frame 1: 1 alloc
  ...
  TEST_CASE("memory::NFrameAllocator::allocate") {
  ...
  TEST_CASE("memory::NFrameAllocator::deallocate") {
  ...
  TEST_CASE("memory::FrameAllocator alias") {
  ...
  TEST_CASE("memory::DoubleFrameAllocator alias") {
  ...
  TEST_CASE("memory::NFrameAllocator thread-safety: concurrent allocate") {
  ...
  TEST_CASE("memory::NFrameAllocator thread-safety: concurrent deallocate") {
