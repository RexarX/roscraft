...
#include <roscraft/memory/allocator_traits.hpp>
...
TEST_SUITE("memory::AllocatorTraitsConcepts") {
  TEST_CASE("memory::AllocatorTraitsConcepts::PmrAllocator") {
  ...
  TEST_CASE("memory::AllocatorTraitsConcepts::PmrAllocatorWithStats") {
    static_assert(PmrAllocatorWithStats<StatsResource>);
    static_assert(!PmrAllocatorWithStats<BareResource>);
    CHECK(true);
  }

  TEST_CASE("memory::AllocatorTraitsConcepts::ResettablePmrAllocator") {
...
TEST_SUITE("memory::TryAllocate") {
  TEST_CASE("memory::TryAllocate::returns typed pointer on success") {
...
TEST_SUITE("memory::TryAllocateSpan") {
  TEST_CASE("memory::TryAllocateSpan::returns empty span for zero count") {
    StatsResource allocator;
  ...
  TEST_CASE("memory::TryAllocateSpan::returns span on success") {
    StatsResource allocator;
  ...
  TEST_CASE("memory::TryAllocateSpan::returns invalid_size on overflow") {
    StatsResource allocator;
...
TEST_SUITE("memory::DeallocateObject") {
  TEST_CASE("memory::DeallocateObject::accepts null pointer") {
    StatsResource allocator;
...
TEST_SUITE("memory::DeallocateSpan") {
  TEST_CASE("memory::DeallocateSpan::accepts empty span") {
    StatsResource allocator;
    std::span<int> empty;
...
TEST_SUITE("memory::StatsFunction") {
  TEST_CASE("memory::StatsFunction::returns allocator-provided stats") {
    StatsResource allocator;
  ...
  TEST_CASE("memory::StatsFunction::returns zero stats for plain allocators") {
